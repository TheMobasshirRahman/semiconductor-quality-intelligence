...
control, failure-signal analysis, fail prediction and equipment-health monitoring, served in an interactive dashboard.
...
| **Failure signals** | A consensus of 5 feature-selection methods plus stability selection narrows 590 sensors to 7 stable core sensors *associated* with failure: a shortlist for engineers to investigate, not proven causes. 30 sensors match the accuracy of all 272 cleaned ones. |
| **Equipment health** | Multivariate SPC (PCA T² / SPE) automatically flagged a sensor reading ≈ 9999 (consistent with a sentinel/error value) and a two-day burst on another sensor, all on passing units, and named the responsible sensor each time. |
| **Engineering** | sklearn pipelines fitted on the training period only, one-command rebuild, 55 automated tests, deterministic reruns in a pinned environment. |
...
- Nested CV (ranking redone inside every fold): 30 sensors → BER 30.9% vs 30.3% with all 272. For context only, the dataset's published baseline is 33.5% BER,
  measured with a different protocol (10-fold CV on all data, kernel ridge classifier), so the numbers are not directly comparable.
...
- October's failed units look **normal** in sensor space (0/24 caught, AUC ≈ 0.5). Together with step 5 this suggests
  (but does not prove) that October's failures are driven by something these 590 sensors do not capture well, so a
  sensible next step is to widen data collection.
- It does catch **equipment/sensor events** on passing units: `sensor_140` reading ≈ 9999 (likely a sentinel/error value,
  not confirmed) and a two-day `sensor_007` burst.
...
Six pages: KPIs and yield trend, per-sensor control charts, failure-signal ranking and drift heatmap, risk bands with
...
are committed, and `python -m src.pipeline` reproduces them (bit-identical with the same platform and package versions).
...
- **Association, not causation:** feature rankings and SHAP values show which signals move with failure in this
  observational data. Confirming a cause needs process knowledge or designed experiments.
- The 30 sensors were selected on the whole training period, so the forward-validation score used to choose the model
  (step 5) is somewhat optimistic. The test-period metrics are not affected.
- The test period was scored more than once during development (a first model-selection protocol and the anomaly
  settings were revised after seeing it); both runs are reported in notebooks 05–06.
- Percentile control limits rest on the ~1–2 most extreme baseline readings, so they are statistically unstable;
  a parametric fit or more baseline data would tighten them.
- The Power BI export (`powerbi/`) still uses an earlier, simpler version of the analysis (random-CV probabilities,
  mean ± 3σ limits) and does not yet match the dashboard numbers.
